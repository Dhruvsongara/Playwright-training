...
test('test @sanity', async ({ page }) => {
  ...
  await expect(page.locator('app-profile')).toContainText('iphone 13 pro');
  await expect(page.locator('app-profile')).toContainText('ADIDAS ORIGINAL');
  await page.getByRole('button', { name: 'Checkout❯' }).click();
  await expect(page.locator('section')).toContainText('dhruv123@gmail.com');
  ...
  await page.getByRole('combobox').first().selectOption('09');
  ...
  await expect(page.locator('#htmlData')).toContainText('ADIDAS ORIGINAL');
});